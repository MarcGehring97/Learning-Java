...
public class Util {
    ...
    /**
     * Creates a new file- or directory name by appending "fileOrDirname"
     * to "dirname".Takes care of the operating-system specific
     * file-separator which has to be in between
     * (e.g., "/" on Linux, but "\" on Windows)
     * @return a new file- or directory name, pointing to
     * the "fileOrDirname" in the directory "dirname"
     */
    ...
    public static void moveFile(String srcFilename, String destFilename) {
        try {
            Files.move(new File(srcFilename).toPath(),
                    new File(destFilename).toPath());
        } catch (IOException e) {
            System.err.println("error writing " + srcFilename
                    + " to " + destFilename);
            System.exit(-1);
        }
    }
    ...
    public static void copyFile(String srcFilename, String destFilename) {
        try {
            Files.copy(new File(srcFilename).toPath(),
                    new File(destFilename).toPath());
        } catch (IOException e) {
            System.err.println("error writing " + srcFilename
                    + " to " + destFilename);
    ...
    /**
     * @return a string which uniquely identifies a point in time
     * (search for "Unix timestamp" for further information)
     */
    ...
    /**
     * @return the names of all files (excluding directories)
